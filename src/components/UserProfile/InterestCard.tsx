...
const InterestCardontainer = styled.div`
  display: flex;
  padding: 10px 15px;
  width: 590px;
  height: 90px;
...
const AddButton = styled.button<FollowTextProps>`
  width: 120px;
  height: 40px;
...
const InterestCard = ({ item }) => {
  ...
  return (
    <InterestCardontainer key={item.id}>
      <TitleContainer>
      ...
      <AddButton added={add ? true : false} onClick={() => setAdd(!add)}>
        {add ? ' Added' : ' Add +'}
      </AddButton>
